...
import { useDisplay, useModals, useResources } from '@/store';
...
const Header = () => {
  ...
  const { addQuery, deleteQuery } = useResources((state) => state);
  ...
  const handleChange = (query: string) => {
    ...
    if (trimmedQuery) {
      return addQuery('products', { query: 'q', value: query });
    }

    deleteQuery('products', 'q');
  };
